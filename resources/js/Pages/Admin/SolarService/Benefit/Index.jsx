...
export default function Index({ benefits }) {
    const handleDelete = (id, title) => {
        if (confirm(`Delete benefit "${title}"?`)) {
            router.delete(`/admin/solar-service/benefit/${id}`);
        }
    };

    return (
        <AuthenticatedLayout>
            <Head title="Solar Service — Benefits" />
            <div className="max-w-5xl mx-auto px-4 py-10">
                <BackToDashboard />
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-2xl font-bold text-[#1A3A5C]">Solar Service — Benefits</h1>
                    <Link
                        href="/admin/solar-service/benefit/create"
                ...
                <TabBar active="benefit" />

                {benefits.length === 0 ? (
                    <p className="text-gray-500 text-sm">No benefits yet.</p>
                ) : (
                        ...
                        {benefits.map((b) => (
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    <p className="font-semibold text-[#1A3A5C] truncate">{b.title}</p>
                                    <p className="text-xs text-gray-500">
                                        Item: <span className="text-gray-700">{b.item?.title ?? "—"}</span>
                                        {" · "}
                                        Order: {b.order} ·{" "}
                                        <span className={b.is_active ? "text-green-600" : "text-red-500"}>
                                ...
                                <div className="flex gap-2 flex-shrink-0">
                                    <Link
                                        href={`/admin/solar-service/benefit/${b.id}/edit`}
